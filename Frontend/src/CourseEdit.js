...
const CourseEdit = () => {
    ...
    const [validation, setValidation]=useState({course_name: "", course_year: "", course_term: "", course_credits: "", course_capacity: "", Prerequisite: ""});
    ...
    const getcourses = async () => {
        try {
            const response = await axios.get("http://localhost:8080/course-1/api/courselist/"+courseid);
...
const getfaculty = async () => {
    try {
        const response = await axios.get("http://localhost:8080/course-1/api/courseupdate/facultylist");
...
//===============================================================================================================================
    ...
    const handleSelect = (e) => {
        ...
        formValues.Prerequisite=getselectValues.filter(item => formValues.course_name !== item)
        formValues.Prerequisite= formValues.Prerequisite.toString()     
    }
...
//=====================================================================================================================
    const handlesubmit =  (e) => {
        ...
        if ((getuserArr && getuserArr.length)) {
            ...
            if(isSubmit){
                    try {
                        const response=  axios.put("http://localhost:8080/course-1/api/courselist/"+courseid, formValues)
                            ...
                            <div className="row">
                                ...
                                {/* { <div className="col-lg-12">
                                    <div className="form-group">
                                        ...
                                        <MultiSelect name="course_prerequisites" isObject={false} disable={show1} selectedValues={formValues.Prerequisite.split(',').filter(item => formValues.course_name !== item)} 
                                ...
                                <div className="col-lg-12">
                                    <div className="form-group">
                                        <label>Faculty</label>
                                        <select required name="dummy" disabled={show1} value={formValues.employee.name} onChange={(e) => handleChange(e)} className="form-control">
                                        {
                                            coursefaculty.map((faculty)=> (
                                            <option key={faculty.employeeid} value={faculty.name}> {faculty.name}</option>
                                            ))
                                        }
                                        </select>
                                    </div>
